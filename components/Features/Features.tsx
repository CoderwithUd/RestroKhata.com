...
function FeatureVisual({ type }: { type: (typeof features)[number]["visual"] }) {
    ...
    return (
      <>
        ...
        <div className={styles.kdsGrid}>
          <article><small>TABLE 4</small><span className={styles.itemName}>Butter Chicken</span><span>Dal Makhani x2</span><span className={styles.status}>PLACED</span></article>
          <article><small>TABLE 7</small><span className={styles.itemName}>Masala Chai x2</span><span>Samosa x4</span><span className={styles.status}>IN PROGRESS</span></article>
        </div>
      </>
    );
  }
    ...
    return (
      <>
        ...
        {["Owner|Full access", "Manager|Orders & reports", "Waiter|Orders & tables", "Kitchen|Queue & availability"].map((role) => {
          ...
          return <div className={styles.role} key={name}><span className={styles.itemName}>{name}</span><span>{desc}</span></div>;
        })}
      </>
    );
  }
  return (
    <>
      ...
      <div className={styles.reportCards}><article><span>Today Revenue</span><span className={styles.reportValue}>₹18,420</span></article><article><span>Orders</span><span className={styles.reportValue}>47</span></article></div>
      ...
      <div className="container">
        ...
        <div className={styles.list}>
          {features.map((feature, index) => (
            <article className={`${styles.block} ${index % 2 ? styles.reverse : ""}`} id={`feature-${feature.visual}`} key={feature.title}>
