                  ...
                  return (
                    <div key={item.id}
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-[13px] font-semibold leading-snug text-slate-900 break-words">{item.name}</p>
                  ...
                  return (
                    <div key={`${entry.itemId}-${entry.variantId}-${(entry.optionIds || []).sort().join(",")}`} 
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <p className="text-[12px] font-bold text-slate-800 break-words">{entry.name}</p>
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50 p-2 text-[11px] text-slate-500">
                {cart.map((e) => (
                  <div key={`${e.itemId}-${e.variantId}`} className="flex justify-between">
                    <span className="break-words">{e.quantity}× {e.name}</span>
                  ...
                  return (
                    <div key={`mob-ta-${entry.itemId}-${entry.variantId}-${(entry.optionIds || []).sort().join(",")}`} 
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="text-base font-bold text-slate-900 leading-tight break-words">{entry.name}</p>
