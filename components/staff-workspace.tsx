...
export function StaffWorkspace({ tenantName }: Props) {
  ...
  const [staffListViewMode, setStaffListViewMode] =
    useState<StaffListViewMode>("grid");
  const [editingMember, setEditingMember] = useState<TenantStaffMember | null>(
    null,
  );
    ...
    <>
            ...
            staffListViewMode === "grid" ? (
              <div className="space-y-2">
                {filteredStaff.map((member) => {
                  ...
                  const isActive = member.isActive ?? true;
                  return (
                    <article key={member.membershipId} className="rounded-xl border border-[#eadfc9] bg-[linear-gradient(160deg,#fffcf6_0%,#fff7e8_100%)] p-2.5 sm:p-3">
                      <div className="grid gap-2 lg:grid-cols-[1.25fr_.9fr_.8fr_.6fr]">
                        ...
                        <p className={`inline-flex h-10 items-center justify-center rounded-lg border px-2.5 text-xs font-semibold ${isActive ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-300 bg-slate-100 text-slate-700"}`}>
              ...
              <div className="no-scrollbar -mx-1 overflow-x-auto overscroll-x-contain px-1 sm:mx-0 sm:px-0">
                <table className="w-full min-w-[760px] divide-y divide-[#efe4d3] rounded-xl border border-[#eadfc9] bg-white text-left text-xs whitespace-nowrap sm:min-w-full">
                      ...
                      return (
                        <tr key={`staff-row-${member.membershipId}`}>
                          ...
                          <td className="px-2.5 py-2 sm:px-3">
                            <span className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-semibold ${isActive ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-300 bg-slate-100 text-slate-700"}`}>
                              {isActive ? "Active" : "Inactive"}
                            </span>
                          </td>
                          <td className="px-2.5 py-2 sm:px-3">
      ...
      {editingMember ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center p-3 sm:p-6">
          ...
          />
          <section className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-[#e6dfd1] bg-[#fffdf9] p-3 shadow-2xl sm:p-5">
            <div className="mb-3 flex items-center justify-between sm:mb-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500">
                  Quick Edit
            ...
            <div className="space-y-3">
              <p className="text-sm font-semibold text-slate-900">
                {editingMember.user.name || "No name"}
              </p>
              ...
              <button
                type="button"
                onClick={async () => {
                  await saveMember(editingMember);
