...
function allItemsDone(order: OrderRecord): boolean {
  ...
  return (
    items.length > 0 &&
    items.every((i) => ["READY", "SERVED"].includes(ns(i.status)))
  );
}

function orderActionKey(
  orderId: string,
  lineId: string,
  status?: string,
): string {
...
function getBulkOrderAction(order: OrderRecord): {
  ...
  const cookingItems = items.filter(
    (item) => ns(item.status) === "IN_PROGRESS",
  );
...
function StatPill({
  ...
  return (
    <div
      className={`flex flex-col items-center rounded-2xl border px-4 py-2.5 min-w-[70px] ${color}`}
    >
...
function LiveBadge({ connected }: { connected: boolean }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-semibold ${connected ? "border-emerald-300 bg-emerald-50 text-emerald-700" : "border-slate-200 bg-slate-100 text-slate-400"}`}
    >
      <span
...
function OrderCard({
  ...
  const moveTargets = allOrders.filter(
    (o) =>
      o.id !== order.id &&
      isActive(o.status) &&
      (o.tableId || o.table?.id) === (order.tableId || order.table?.id),
  );
  const tableTargets = tables.filter(
    (t) => t.id !== (order.tableId || order.table?.id),
  );

  const tokenLabel = order.orderNumber
    ? `#${order.orderNumber}`
    : `#${order.id.slice(-6)}`;
  const tableLabel =
    order.table?.name ||
    (order.table?.number ? `Table ${order.table.number}` : null);
  const customerLabel =
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.04)]">
      {/* Order header */}
      <div
        className={`flex flex-wrap items-start justify-between gap-2 px-4 py-3 ${isTakeaway ? "bg-violet-50/60 border-b border-violet-100" : "bg-amber-50/60 border-b border-amber-100"}`}
      >
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            {/* Mode badge */}
            <span
              className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-bold ${isTakeaway ? "border-violet-300 bg-violet-100 text-violet-700" : "border-amber-300 bg-amber-100 text-amber-700"}`}
            >
              {isTakeaway ? (
                <svg
                  viewBox="0 0 16 16"
            ...
            <span
              className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase ${orderBadgeClass(order.status)}`}
            >
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {readyItems.length > 0 && (
            <button
              ...
              className="rounded-xl border border-emerald-300 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-800 transition hover:bg-emerald-100 disabled:opacity-50"
            >
              {servingItemKey === `batch:${order.id}:SERVED`
                ? "Serving..."
      ...
      {items.length > 0 && (() => {
        ...
        return (
          <div className="divide-y divide-slate-100">
            ...
            const servingKey = item.lineId
              ? orderActionKey(order.id, item.lineId, next)
              : null;
    ...
    >
      {STATUS_LABELS[ns(item.status)] || item.status || "Pending"}
                  ...
                  {/* Remove */}
                  {item.lineId && canCorrect(item.status) && item.quantity > 1 &&(
                    <button
                      type="button"
                      disabled={isCorrecting}
                      onClick={() => onRemoveItem(order, item, corrVal)}
...
export function OrdersBoardView() {
  ...
  const activeOrders = useMemo(
    () =>
      allOrders.filter(
        (o) => !invoicedOrderIds.has(o.id) && isActive(o.status),
      ),
  ...
  const shownOrders =
    sectionFilter === "dine-in"
      ? dineInOrders
      : sectionFilter === "takeaway"
        ? takeawayOrders
        : activeOrders;
  ...
  const cookingCount = activeOrders.filter((o) =>
    ["PLACED", "IN_PROGRESS"].includes(ns(o.status)),
  ).length;
  const billingCount = activeOrders.filter(
    (o) => canInvoice(o.status) || allItemsDone(o),
  ).length;
  ...
  async function handleMarkItemServed(order: OrderRecord, item: OrderItem) {
    ...
    try {
      ...
      await updateOrder({
        orderId: order.id,
        payload: { itemStatusUpdates: [{ lineId: item.lineId, status: next }] },
      }).unwrap();
  ...
  async function handleMarkReadyAll(order: OrderRecord) {
    const ready = activeItems(order).filter(
      (i) => ns(i.status) === "READY" && i.lineId,
    );
    ...
    try {
      ...
      await updateOrder({
        orderId: order.id,
        payload: {
          itemStatusUpdates: ready
            .filter((i): i is OrderItem & { lineId: string } =>
              Boolean(i.lineId),
            )
            .map((i) => ({
              lineId: i.lineId,
        ...
        await updateOrder({
          ...
          payload: {
            itemStatusUpdates: targetItems
              ...
              .map((item) => ({
                lineId: item.lineId,
                status: bulkAction.targetStatus as OrderStatus,
              })),
            ...(willCompleteOrder
  ...
  async function handleRemoveItem(
    ...
    try {
      ...
      await removeOrderItem({
        orderId: order.id,
        lineId: item.lineId,
        payload: q < item.quantity ? { quantity: q } : undefined,
      }).unwrap();
